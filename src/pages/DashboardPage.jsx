...
export default function DashboardPage() {
  ...
  if (data.lowStock.length > 0) {
    alerts.push({
      type: "warning",
      text: `${data.lowStock.length} produto(s) com estoque baixo`,
    });
  }

  if (data.openOrders.length > 10) {
    alerts.push({
      type: "warning",
      text: `${data.openOrders.length} ordens em aberto exigem acompanhamento`,
    });
  }
  ...
  if (data.readyOrders.length > 0) {
    alerts.push({
      type: "success",
      text: `${data.readyOrders.length} OS pronta(s) para entrega`,
    });
  }

  if (data.payableTodayTotal > data.cashIn) {
    alerts.push({
      type: "danger",
      text: "Saídas previstas maiores que as entradas no caixa hoje",
    });
  }
  ...
  const healthScore = Math.max(
    ...
    Math.min(
      ...
      100 -
        ...
        data.openOrders.length * 2 -
        (data.payableTodayTotal > data.cashIn ? 20 : 0)
    )
  );
    ...
    <div className="page-stack dashboard-page">
      ...
      {alerts.length > 0 && (
        <section className="dashboard-alerts">
          {alerts.map((alert, index) => (
            <div key={`${alert.type}-${index}`} className={`dashboard-alert dashboard-alert-${alert.type}`}>
              {alert.text}
            </div>
          ))}
        </section>
      ...
      <Card>
        <div className="dashboard-section-head dashboard-section-head--spaced">
          <div>
            ...
            <p className="dashboard-section-subtitle">
              Atalhos para iniciar as rotinas mais frequentes da operação.
            </p>
          </div>
        </div>
        ...
        <div className="quick-actions">
          ...
          <Button onClick={() => navigate("/ordens-servico")}>Nova OS</Button>
          <Button variant="secondary" onClick={() => navigate("/financeiro")}>Receber</Button>
          <Button variant="secondary" onClick={() => navigate("/caixa")}>Abrir caixa</Button>
        </div>
      </Card>
      ...
      <section className="dashboard-overview-grid">
        ...
        <div className="dashboard-side-stack">
          <Card>
            <div className="dashboard-section-head dashboard-section-head--spaced">
              <div>
                <h3 className="dashboard-section-title">Saúde da operação</h3>
                <p className="dashboard-section-subtitle">
                  Índice rápido baseado em estoque, ordens abertas e equilíbrio do caixa do dia.
                </p>
              </div>
              <span className={`pill ${healthScore >= 80 ? "pill-success" : healthScore >= 60 ? "pill-warning" : "pill-danger"}`}>
                {healthScore}%
              </span>
            </div>
            ...
            <div className="health-meta">
              {healthScore >= 80
                ? "Operação estável para hoje"
                : healthScore >= 60
                  ? "Atenção em pontos operacionais"
                  : "Dia exige ação rápida"}
            </div>
          </Card>
