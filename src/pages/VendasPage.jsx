...
function parsePaymentNotes(paymentNotes) {
  ...
  if (start === -1 || end === -1 || end <= start) {
    return { note: raw, splitEnabled: false, paymentSplits: [] };
  }
  ...
  try {
    ...
    return {
      note: parsed?.userNotes || note,
      splitEnabled: Boolean(parsed?.splitEnabled),
      paymentSplits: Array.isArray(parsed?.paymentSplits) ? parsed.paymentSplits : [],
    };
  } catch {
    return { note: note || raw, splitEnabled: false, paymentSplits: [] };
  }
}
...
export default function VendasPage() {
  ...
  const paymentDetails = useMemo(
    () => parsePaymentNotes(selectedSale?.payment_notes),
    [selectedSale?.payment_notes]
  );
              ...
              <>
                ...
                {activeTab === "resumo" && (
                  <div className="summary-grid">
                    ...
                    <div><strong>Obs. pagamento:</strong> {paymentDetails.note || "—"}</div>
                    {paymentDetails.splitEnabled && paymentDetails.paymentSplits.length > 0 && (
                      <div style={{ gridColumn: "1 / -1" }}>
                        <strong>Recebimentos:</strong>
                        <div className="payment-split-list">
                          {paymentDetails.paymentSplits.map((split, index) => (
                            <div key={`${split.method}-${index}`} className="payment-split-item">
                              <span>{split.method || "Forma"}</span>
                              <strong>{currencyBR(split.amount || 0)}</strong>
                            </div>
                          ))}
                        </div>
                      </div>
                ...
                {activeTab === "pagamento" && (
                  <div className="summary-grid">
                    ...
                    <div>
                      <strong>Observações:</strong> {paymentDetails.note || "—"}
                    </div>
                    {paymentDetails.splitEnabled && paymentDetails.paymentSplits.length > 0 && (
                      <div style={{ gridColumn: "1 / -1" }}>
                        <strong>Recebimentos:</strong>
