...
function serializePaymentMetadata(userNotes, paymentSplits, splitEnabled) {
  ...
  if (!splitEnabled || validSplits.length <= 1) {
    return note || null;
  }
...
function validateStatusTransition(order) {
  ...
  const payment = summarizePayment(totals.total, order.amountPaid, order.paymentSplits, order.paymentSplitEnabled);
  const hasService =
    ...
    <div className="os-timeline">
      {steps.map((step, index) => {
        ...
        return (
          <div
            key={step}
            className={`os-timeline-step ${isDone ? "is-done" : ""} ${isCurrent ? "is-current" : ""}`}
          >
...
function OsStatusHero({ status, paymentLabel, remaining, technician, openedAt }) {
  return (
    <div className="os-status-hero">
      <div className="os-status-hero__main">
        <div className="os-status-hero__label">Status atual</div>
        <div className="os-status-hero__row">
          <span className={`pill ${STATUS_COLORS[status] || ""}`}>{status || "—"}</span>
          <strong>{technician || "Técnico não definido"}</strong>
        </div>
      </div>

      <div className="os-status-hero__meta">
        <div>
          <span>Pagamento</span>
          <strong>{paymentLabel || "Não definido"}</strong>
        </div>
        <div>
          <span>Saldo</span>
          <strong>{currencyBR(remaining)}</strong>
        </div>
        <div>
          <span>Abertura</span>
...
async function buildPdf(order, settings = {}) {
  ...
  section("PAGAMENTO");
  const payment = summarizePayment(totals.total, order.amountPaid, order.paymentSplits, order.paymentSplitEnabled);
  field("Forma", getPaymentMethodLabel(order.paymentMethod, order.paymentSplits, order.paymentSplitEnabled));
  field("Recebido", currencyBR(payment.received));
  field("Restante", currencyBR(payment.remaining));
  if (order.paymentSplitEnabled) {
    getPaymentSplitEntries(order.paymentSplits).forEach((split) => {
      field(`• ${split.method || "Forma"}`, currencyBR(parseMoney(split.amount)));
    });
  }
...
async function buildCouponPdf(order, settings = {}) {
  ...
  labelValue("Forma:", getPaymentMethodLabel(order.paymentMethod, order.paymentSplits, order.paymentSplitEnabled) || "—");
  labelValue("Recebido:", currencyBR(payment.received));
  labelValue("Restante:", currencyBR(payment.remaining), true);
  if (order.paymentSplitEnabled) {
    getPaymentSplitEntries(order.paymentSplits).forEach((split) => {
      labelValue(`• ${split.method || "Forma"}:`, currencyBR(parseMoney(split.amount)));
    });
  }
...
export default function OrdensServicoPage() {
  ...
  const filteredOrders = useMemo(() => {
    ...
    return [...orders]
      .filter((order) => {
        ...
        const payment = summarizePayment(orderTotal, order.amountPaid, order.paymentSplits, order.paymentSplitEnabled);
  ...
  const orderFlags = useMemo(() => {
    ...
    const payment = summarizePayment(total, form.amountPaid, form.paymentSplits, form.paymentSplitEnabled);
  ...
  const osChecklist = useMemo(() => {
    return [
      { label: "Cliente", done: !!String(form.clientName || "").trim() },
      {
        label: "Equipamento",
        done:
          !!String(form.equipmentType || "").trim() ||
          !!String(form.equipmentModel || "").trim(),
      },
      { label: "Defeito", done: !!String(form.issue || "").trim() },
      { label: "Técnico", done: !!String(form.tecnico || "").trim() },
      { label: "Pagamento", done: summarizePayment(totals.total, form.amountPaid, form.paymentSplits, form.paymentSplitEnabled).received > 0 },
    ];
  }, [form, totals.total]);

  const pendingChecklist = useMemo(
    () => osChecklist.filter((item) => !item.done).map((item) => item.label),
    [osChecklist]
  );
  ...
  usePageHotkeys(
    [
      { combo: "primary+s", handler: () => handleSave(), allowInInput: true },
      { combo: "primary+n", handler: () => handleNew(), allowInInput: true },
      { combo: "/", handler: () => focusSearchInput() },
      {
        combo: "escape",
        handler: () => {
      ...
      { combo: "primary+1", handler: () => setActiveTab("atendimento"), allowInInput: true },
      { combo: "primary+2", handler: () => setActiveTab("servicos"), allowInInput: true },
      { combo: "primary+3", handler: () => setActiveTab("financeiro"), allowInInput: true },
      { combo: "primary+4", handler: () => setActiveTab("historico"), allowInInput: true },
    ],
  ...
  async function upsertReceivableForOrder(payload, payment, total) {
    ...
    const { error: receivableError } = await supabase.from("receivables").upsert({
      ...
      notes: form.paymentNotes || "Gerado automaticamente pela OS.",
  ...
  async function handleSave() {
    ...
    const payment = summarizePayment(previewTotals.total, form.amountPaid, form.paymentSplits, form.paymentSplitEnabled);
    ...
    try {
      const payload = normalizeOrder(
      {
        ...form,
        tenant_id: tenantId,
        paymentMethod: getPaymentMethodLabel(form.paymentMethod, form.paymentSplits, form.paymentSplitEnabled),
        paymentNotes: serializePaymentMetadata(form.paymentNotes, form.paymentSplits, form.paymentSplitEnabled),
        amountPaid: String(payment.paid || payment.received || ""),
    ...
    return (
      <div className="page-stack">
        <LoadingState
          ...
          description="Estamos sincronizando clientes, estoque, serviços e histórico da oficina."
        />
      </div>
    );
  }
  ...
  return (
    <div className="page-stack os-redesign-page">
      <PageHeader
        ...
        description="Organize atendimento, execução técnica, estoque aplicado e cobrança em um único fluxo."
        action={
      ...
      <StatusBanner
        tone={partWarnings.length ? "warning" : pendingChecklist.length ? "info" : "success"}
        title={
          partWarnings.length
            ? "Revise estoque antes de salvar"
            : pendingChecklist.length
              ? "Checklist rápido da OS"
              : "OS pronta para seguir o fluxo"
        }
        description={
          partWarnings.length
            ? `${partWarnings.length} item(ns) com estoque insuficiente.`
            : pendingChecklist.length
              ? `Pendências atuais: ${pendingChecklist.join(", ")}.`
              : "Cliente, equipamento e defeito já foram informados. Você pode avançar com mais segurança."
        }
        actions={<div className="hotkey-hint">Ctrl+S salvar • Ctrl+N nova • / buscar • Ctrl+1..4 abas</div>}
      >
        <div className="status-checklist">
          {osChecklist.map((item) => (
            <span key={item.label} className={`status-check ${item.done ? "is-done" : "is-pending"}`}>
        ...
        <div className="right-column">
          ...
          {activeTab === "atendimento" && (
            <Card>
              ...
              <OsTimeline status={form.status} />

              <OsStatusHero
                status={form.status}
                paymentLabel={getPaymentMethodLabel(form.paymentMethod, form.paymentSplits, form.paymentSplitEnabled)}
                remaining={paymentSummary.remaining}
                technician={form.tecnico || form.atendente}
                openedAt={formatDateTime(form.createdAt)}
              />

              <div className="os-hero-strip">
                <div className="os-hero-block highlight">
                  <span className="os-hero-label">Status</span>
                  <strong>{form.status || "—"}</strong>
                </div>
                ...
                <div className="os-hero-block">
                  ...
                  <strong>{getPaymentMethodLabel(form.paymentMethod, form.paymentSplits, form.paymentSplitEnabled) || "Não definido"}</strong>
                </div>
              </div>
              ...
              <div className="os-tracking-card">
                <strong className="os-tracking-card__title">Acompanhamento do cliente</strong>

                <div className="os-tracking-card__content">
                  <div className="os-tracking-card__qr">
                    {trackingQrDataUrl ? (
                      <img src={trackingQrDataUrl} alt="QR code de acompanhamento" width={110} height={110} />
                    ) : (
                      <div className="os-tracking-card__qr-placeholder" />
                    )}
                  </div>

                  <div className="os-tracking-card__actions">
                    <input
                      className="os-tracking-card__input"
          ...
          {activeTab === "financeiro" && (
            <div className="os-finance-grid">
              <Card>
                      ...
                      onChange={(v) =>
                        setForm((prev) => ({
                          ...
                          paymentSplits: prev.paymentSplitEnabled
                            ? prev.paymentSplits
                            : [createPaymentSplit(prev.paymentMethod || "Dinheiro", v)],
                        }))
                      }
                ...
                <div className="checkout-options os-finance-options">
                  <label className={`option-card ${form.paymentSplitEnabled ? "is-active" : ""}`}>
                    ...
                    <div className="option-card-content">
                      <span className="option-card-title">
                        Receber com várias formas de pagamento
                      </span>
                      <span className="option-card-description">
                        Divida o recebimento entre dinheiro, Pix, cartão e outras formas.
                      </span>
                    </div>
                  </label>
                </div>

                <div className="os-payment-breakdown-card">
                  <div className="os-payment-breakdown-card__row">
                    <span>Forma consolidada</span>
                  ...
                  <div className="os-payment-breakdown-card__row os-payment-breakdown-card__row--muted">
                ...
                {form.paymentSplitEnabled && (
                  <div className="pdv-split-payments" style={{ marginTop: 12 }}>
                    ...
                    <div className="pdv-split-toolbar">
                      ...
                      <div className="pdv-split-summary">
                        ...
                        <strong>{currencyBR(paymentSummary.paid)}</strong>
                      </div>
                    </div>
                  </div>
                )}
